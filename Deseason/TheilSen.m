...
if sz(2)==2         % normal 2-D case
    ...
    for i=1:sz(1)-1
        % accumulate slopes
        C(i,i+1:end) = (data(i,2)-data(i+1:end,2))./(data(i,1) - data(i+1:end,1));
    end
    ...
    if nargout>=4
        ...
        tempStd = []; 
        for iT = 1:100
            % estimate variance of mean slope and intercept by bootstrap
            rndSet1 = datasample(1:sz(1),500)';
            rndSet2 = datasample(1:sz(1),500)';
            rndPairs = [rndSet1,rndSet2]; % sort chronologically
            rndSetSorted = [min(rndPairs,[],2),max(rndPairs,[],2)];
            noDuplicates = find(rndSetSorted(:,1)~=rndSetSorted(:,2));
            rndSetSorted = rndSetSorted(noDuplicates,:);
            
            subSamp = (data(rndSetSorted(:,2),2)-data(rndSetSorted(:,1),2))./(data(rndSetSorted(:,2),1) - data(rndSetSorted(:,1),1));
            tempMean(iT,1) = nanmedian(subSamp);
            tempStd(iT,1) = nanstd(subSamp);
            
        end
        
        
        p = prctile(tempMean,[5,95]); % calculate 95% confidence interval slope
        pb = [];
